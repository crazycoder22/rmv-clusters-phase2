...
export default function TambolaJoin() {
  ...
  return (
    <div className="flex flex-1 flex-col px-4 pt-[env(safe-area-inset-top,0px)] pb-[max(1rem,env(safe-area-inset-bottom,0px))]">
      <header className="flex items-center justify-between py-4">
        ...
        <h1 className="text-lg font-semibold text-white">Tambola</h1>
        <div className="h-9 w-9" />
      </header>

      <section className="mb-6 rounded-2xl border border-slate-700 bg-slate-800/60 p-4">
        <h2 className="text-sm font-semibold text-slate-200">Have a code?</h2>
      ...
      <section>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-700 bg-slate-800/40 py-10 text-center">
            <Ticket size={32} className="mx-auto mb-2 text-slate-600" />
          ...
          <div className="space-y-2">
            {joinable.map((s) => (
              <button
                key={s.id}
                onClick={() => navigate(`/tambola/${s.code}`)}
