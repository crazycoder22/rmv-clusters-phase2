...
export default function MarketplaceWishlist() {
  ...
  useEffect(() => {
    if (!token) return;
    apiFetch("/api/marketplace/wishlist", { token })
  ...
  return (
    <div className="one-surface flex flex-1 flex-col px-[18px] pt-[env(safe-area-inset-top,0px)] pb-8" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <header className="flex items-center gap-2 py-3">
        ...
        <h1 className="text-[20px] font-extrabold tracking-tight" style={{ color: "var(--text)" }}>Wishlist</h1>
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-[16px] px-4 py-10 text-center" style={{ border: "1px solid var(--border)" }}>
          <p className="text-[14px]" style={{ color: "var(--text-3)" }}>No saved listings yet.</p>
          <Link to="/marketplace" className="mt-2 inline-block text-[13px] font-semibold" style={{ color: "var(--accent)" }}>Browse marketplace</Link>
        </div>
      ) : (
