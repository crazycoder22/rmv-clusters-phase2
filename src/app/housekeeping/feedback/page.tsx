    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((s) => (
        <button
          ...
          type="button"
          onClick={() => onChange(s)}
          className={`text-2xl transition-colors ${s <= value ? "text-yellow-400" : "text-gray-400"}`}
        >
          ★
        </button>
      ))}
...
export default function HousekeepingFeedbackPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Housekeeping Feedback</h1>
      ...
      <div className="flex flex-wrap gap-3">
        <div>
          ...
          <select
            ...
            className="border rounded px-3 py-1.5 text-sm disabled:bg-gray-50"
          >
      ...
      <div className="bg-gray-50 rounded-lg p-4 border">
        <h2 className="text-sm font-semibold text-gray-700 mb-2">Assigned Staff — Block {block}, {MONTHS[month - 1]} {year}</h2>
        {staffForBlock.length === 0 ? (
          <p className="text-sm text-gray-400">No staff assigned for this block this month.</p>
        ) : (
      ...
      {success ? (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-green-800 text-sm">
          ...
          <button className="underline text-green-700 ml-1" onClick={() => setSuccess(false)}>Submit another</button>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-5 space-y-4">
          <h2 className="text-sm font-semibold text-gray-700">Submit Feedback for Block {block}</h2>
          ...
          {!session?.user && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  required value={guestName} onChange={(e) => setGuestName(e.target.value)}
                  className="w-full border rounded px-3 py-1.5 text-sm" placeholder="Full name"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Phone *</label>
                <input
                  required value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)}
                  className="w-full border rounded px-3 py-1.5 text-sm" placeholder="Mobile number"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full border rounded px-3 py-1.5 text-sm" placeholder="Optional"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Flat Number *</label>
                <input
                  required value={guestFlat} onChange={(e) => setGuestFlat(e.target.value)}
                  className="w-full border rounded px-3 py-1.5 text-sm" placeholder="e.g. 301"
                />
              </div>
            </div>
          )}
          ...
          <button
            ...
            className="bg-blue-600 text-white text-sm font-medium px-5 py-2 rounded hover:bg-blue-700 disabled:opacity-60"
          >
      ...
      {loading ? (
        ...
        <div>
          ...
          <ul className="space-y-3">
            {feedbackList.map((f) => {
              ...
              return (
                <li key={f.id} className="bg-white border rounded-lg p-4">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{new Date(f.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
                </li>
              );
            })}
          </ul>
        ...
        <p className="text-sm text-gray-400">No feedback yet for this block and month.</p>
      )}
    </div>
  );
}
