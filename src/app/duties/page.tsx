    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Tick off today&apos;s duties as you complete them</p>
        </div>
                ...
                <div key={cl.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    <div>
                      ...
                      {cl.description && <p className="text-xs text-gray-500 mt-0.5">{cl.description}</p>}
                    </div>
                    <span className={`text-xs font-semibold rounded-full px-2 py-0.5 ${allDone ? "text-green-700 bg-green-100" : "text-gray-600 bg-gray-100"}`}>
                    ...
                    {cl.items.map((it) => (
                      <li key={it.id}>
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-gray-50 disabled:opacity-60"
                        >
                          {it.doneToday ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm ${it.doneToday ? "text-gray-400 line-through" : "text-gray-800"}`}>{it.title}</p>
