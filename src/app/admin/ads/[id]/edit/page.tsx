...
export default function EditAdPage() {
  ...
  const fetchAd = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      const ad = data.ads.find((a: { id: string }) => a.id === id);
      if (ad) {
        setTitle(ad.title);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required />
            </div>
            ...
            {/* Link */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Click-through URL *</label>
              ...
              <div className="flex gap-3">
                {(["top", "bottom"] as const).map((p) => (
                  <button key={p} type="button" onClick={() => setPlacement(p)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${placement === p ? "bg-primary-600 text-white border-primary-600" : "bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-primary-400"}`}>
            ...
            {/* Pages */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Show on Pages *</label>
              <div className="flex flex-wrap gap-2">
                {PAGE_OPTIONS.map((opt) => (
                  <button key={opt.value} type="button" onClick={() => togglePage(opt.value)}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${pages.includes(opt.value) ? "bg-primary-600 text-white border-primary-600" : "bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-primary-400"}`}>
                    {opt.label}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm" required />
              </div>
              <div>
