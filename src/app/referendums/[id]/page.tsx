    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between">
          <Link href="/referendums" className="inline-flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900"><ArrowLeft size={16} /> All referendums</Link>
          {data.canManage && (
            <div className="flex items-center gap-3">
              {data.isOpen && data.turnout === 0 && (
                <Link href={`/referendums/${id}/edit`} className="text-gray-400 dark:text-gray-500 hover:text-blue-600"><Pencil size={18} /></Link>
              )}
              {data.isOpen && (
              ...
              {data.turnout === 0 && (
                <button type="button" onClick={deleteReferendum} className="text-gray-400 dark:text-gray-500 hover:text-red-600"><Trash2 size={18} /></button>
              )}
            </div>
          )}
        </div>

        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-3 flex items-start gap-2">
        ...
        {/* Turnout */}
        <div className="mt-3 inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
        ...
        {data.canManage && (
          <div className="mt-3">
            ...
            {rosterOpen && (
              <div className="mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      {roster.voted.length === 0 ? (
                        <p className="text-sm text-gray-400 dark:text-gray-500">No one yet.</p>
                      ) : (
                        <ul className="space-y-1.5 max-h-72 overflow-y-auto">
                          {roster.voted.map((v, i) => (
                            <li key={i} className="flex items-center justify-between text-sm">
                              <span className="text-gray-800 dark:text-gray-200"><CheckCircle2 size={12} className="inline text-green-600 mr-1" />{v.name}</span>
                              <span className="text-xs text-gray-400 dark:text-gray-500">B{v.block ?? "—"}, {v.flatNumber} · {fmtDateTime(v.votedAt)}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <div>
                      ...
                      {roster.pending.length === 0 ? (
                        <p className="text-sm text-gray-400 dark:text-gray-500">Every eligible flat has voted. 🎉</p>
                      ) : (
                        <ul className="space-y-1.5 max-h-72 overflow-y-auto">
                          {roster.pending.map((p, i) => (
                            <li key={i} className="flex items-center justify-between text-sm">
                              <span className="text-gray-600 dark:text-gray-300"><Clock size={12} className="inline text-amber-500 mr-1" />{p.residents.join(", ")}</span>
                              <span className="text-xs text-gray-400 dark:text-gray-500">B{p.block ?? "—"}, {p.flatNumber}</span>
                            </li>
                          ))}
                        </ul>
                      )}
        ...
        {canVote && (
          <div className="mt-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Cast your flat&apos;s vote</h2>
            <div className="space-y-2">
              {data.options.map((o) => (
                <label
                  key={o.id}
                  className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer ${selected === o.id ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
                >
                  <input
                    type="radio"
                    name="option"
                  ...
                  />
                  <span className="text-sm text-gray-800 dark:text-gray-200">{o.text}</span>
                </label>
              ))}
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-3">Your vote is secret and final — it can&apos;t be changed once submitted.</p>
            <button
              type="button"
        ...
        {data.isOpen && !canVote && (
          <div className="mt-6">
            {data.myFlatVoted ? (
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 rounded-lg p-4 inline-flex items-center gap-2 text-green-800">
                <CheckCircle2 size={18} /> Your flat has voted. Results will be shown after voting closes.
              </div>
            ) : (
              <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 rounded-lg p-4 inline-flex items-center gap-2 text-amber-800">
        ...
        {data.resultVisible && (
          <div className="mt-6">
            <h2 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Result</h2>
            {data.turnout === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">No votes were cast.</p>
            ) : (
              <div className="space-y-3">
                    ...
                    return (
                      <div key={o.id}>
                        <div className="flex items-center justify-between text-sm mb-1">
                          <span className={`flex items-center gap-1 ${isWinner ? "font-bold text-gray-900 dark:text-gray-100" : "text-gray-700 dark:text-gray-300"}`}>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 pt-1">{data.totalVotes} total votes · secret ballot</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-sm w-full p-5">
            <div className="flex items-start justify-between">
              <h3 className="font-bold text-gray-900 dark:text-gray-100">Confirm your vote</h3>
              <button type="button" onClick={() => setConfirmOpen(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600"><X size={18} /></button>
            </div>
            <p className="text-sm text-gray-700 dark:text-gray-300 mt-3">
              You are voting for:
            </p>
            <p className="text-base font-bold text-gray-900 dark:text-gray-100 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 rounded-lg px-3 py-2 mt-1">
              {selectedOption.text}
            </p>
            ...
            <div className="flex gap-2 mt-4">
              <button type="button" onClick={() => setConfirmOpen(false)} className="flex-1 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg py-2.5 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700">Go back</button>
