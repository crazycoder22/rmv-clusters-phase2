    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between">
          <Link href="/initiatives" className="inline-flex items-center gap-1 text-sm text-gray-600 hover:text-gray-900"><ArrowLeft size={16} /> All initiatives</Link>
          {data.canManage && (
            <div className="flex items-center gap-3">
              <Link href={`/initiatives/${id}/edit`} className="text-gray-400 hover:text-blue-600"><Pencil size={18} /></Link>
              <button type="button" onClick={deleteInitiative} className="text-gray-400 hover:text-red-600"><Trash2 size={18} /></button>
            </div>
          )}
        </div>

        <h1 className="text-2xl font-bold text-gray-900 mt-3 flex items-start gap-2"><Megaphone className="text-blue-600 mt-1 shrink-0" size={22} /> {data.title}</h1>
        <p className="text-xs text-gray-500 mt-1">by {data.author.name} · Block {data.author.block ?? "—"}, {data.author.flatNumber}</p>
        ...
        {/* Feedback list */}
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mt-6 mb-3 flex items-center gap-1.5">
          <MessageSquare size={14} /> Feedback ({data.comments.length})
        </h2>

        {data.comments.length === 0 ? (
          <p className="text-sm text-gray-400">No feedback yet.{data.isOpen ? " Be the first to share your thoughts." : ""}</p>
        ) : (
        ...
        {data.isOpen ? (
          ...
          <p className="mt-6 text-sm text-gray-400 text-center">Commenting is closed for this initiative.</p>
        )}
      </div>
    </div>
  );
...
function CommentCard({
  ...
  return (
    <div className={`bg-white border rounded-lg p-3 ${c.isOfficial ? "border-blue-200 bg-blue-50/40" : "border-gray-200"}`}>
      <div className="flex items-center gap-2">
        {compact && <CornerDownRight size={13} className="text-gray-300 shrink-0" />}
        <span className="text-sm font-semibold text-gray-900">{c.author.name}</span>
        <span className="text-xs text-gray-400">Block {c.author.block ?? "—"}</span>
        {c.isOfficial && (
      ...
      <div className="flex items-center gap-4 mt-2">
        <button
          type="button"
          onClick={onLike}
          className={`inline-flex items-center gap-1 text-xs ${c.myLiked ? "text-red-600" : "text-gray-400 hover:text-gray-600"}`}
        >
        ...
        {onDelete && (
          <button type="button" onClick={onDelete} className="text-xs text-gray-400 hover:text-red-600 ml-auto"><Trash2 size={13} /></button>
        )}
        <span className="text-[11px] text-gray-300 ml-auto">{timeAgo(c.createdAt)}</span>
      </div>
    </div>
  );
}
