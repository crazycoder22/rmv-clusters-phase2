  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
        <Link href={editing ? `/parking/${slotId}` : "/parking"} className="inline-flex items-center gap-1 text-sm text-gray-600 hover:text-gray-900"><ArrowLeft size={16} /> Back</Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-3 mb-6">{editing ? "Edit slot" : "List your parking slot"}</h1>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
          ...
          <div className="pt-2 border-t border-gray-100">
            ...
            <Field label="Payment QR image" optional>
              {payQrUrl ? (
                <div className="relative inline-block">
                  ...
                  <img src={payQrUrl} alt="Payment QR" className="h-32 w-32 object-contain bg-white rounded-lg border border-gray-200 p-1" />
            ...
            <p className="text-xs text-gray-400">Bookers see these to pay you directly. Payment stays offline.</p>
          </div>
          ...
          {editing && (
            <label className="flex items-center gap-2 pt-2 border-t border-gray-100">
              ...
              <span className="text-sm text-gray-700">Listed (accepting bookings)</span>
            </label>
          )}
...
function Field({ label, optional, children }: { label: string; optional?: boolean; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">{label}{optional && <span className="text-gray-400 font-normal"> (optional)</span>}</label>
      {children}
    </div>
  );
}
